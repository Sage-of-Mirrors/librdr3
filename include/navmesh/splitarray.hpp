...
public:
    TSplitArray() : mTotalSize(0), bSerializedData(false) { mBuckets.push_back(shared_vector()); }
    ~TSplitArray() { }
    ...
    void Deserialize(bStream::CStream* stream, std::function<void(bStream::CStream*, std::shared_ptr<T>)> elementDeserializeFunc) {
        uint64_t offset = StreamUtil::DeserializePtr64(stream);
        ...
        uint64_t dataTableOffset  = StreamUtil::DeserializePtr64(stream);
        uint64_t indexTableOffset = StreamUtil::DeserializePtr64(stream);
        ...
        for (uint32_t tableIdx = 0; tableIdx < dataTableCount; tableIdx++) {
            uint64_t dataOffset = StreamUtil::DeserializePtr64(stream);
    ...
    uint64_t SerializeMetadata(bStream::CStream* stream, uint32_t id) {
        ...
        StreamUtil::SerializePtr64(stream, bucketInfoOffset); // Bucket info pointer
        StreamUtil::SerializePtr64(stream, indexInfoOffset);  // Index info pointer
        ...
        // Write bucket info
        for (size_t i = 0; i < mBuckets.size(); i++) {
            StreamUtil::SerializePtr64(stream, mDataOffsets[i]); // Offset to the bucket's data
